...
<template>
  <div class="flex items-center rounded-lg bg-white shadow-lg overflow-hidden">
    <img
      ...
      :src="require(`~/assets/img/${destination.imageUrl}`)"
      :alt="destination.imageAlt"
    />
    <div class="px-6 py-4">
      <h3 class="text-lg font-semibold text-gray-800">{{ destination.city }}</h3>
      <p class="text-gray-900">${{ destination.averagePrice }} / night average</p>
      <div class="mt-4">
        <nuxt-link
          to="/property"
          class="text-indigo-500 hover:text-indigo-400 font-semibold text-sm"
        >Explore {{ destination.propertyCount }} properties</nuxt-link>
      </div>
    </div>
  </div>
</template>
